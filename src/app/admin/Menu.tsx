...
const CustomMenu = () => {
  ...
  const totalResources = displayableResources.length;
  ...
  const rootResources = displayableResources.filter(
    (resource) => !resources[resource].options?.menu
  );

  const resourcesByMenu = displayableResources.reduce((acc, resource) => {
