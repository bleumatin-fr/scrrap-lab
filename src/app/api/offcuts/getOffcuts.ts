...
const getOffcuts = async (request: NextRequest, audience?: string[]) => {
  ...
  if (request.nextUrl.searchParams.has("referenceEnd")) {
    const referenceEnd = request.nextUrl.searchParams.get("referenceEnd");
    filters = {
      ...filters,
      reference: new RegExp(`${referenceEnd}$`, "i"),
    };
  }
  ...
  if (request.nextUrl.searchParams.has("qualities")) {
    const qualities = request.nextUrl.searchParams.get("qualities");
    filters = { ...filters, qualities: { $in: qualities } };
  }
  ...
  if (request.nextUrl.searchParams.has("_end")) {
    limit = parseInt(request.nextUrl.searchParams.get("_end") || "10");
  }
